    ...
    with md_pose.Pose(
        ...
        while cap.isOpened():
            ...
            if len(imlist)!=0: #check for if human is within the camera
                if(imlist[15][2] < imlist[12][2] and imlist[16][2] < imlist [12][2]): #if both wrists below left shoulder
                    position="down"
                    countingStatus = False
                if(imlist[15][2]>imlist[11][2] and imlist[15][1] > imlist[12][1] and imlist[16][2]>imlist[11][2] and imlist[16][1] > imlist[12][1]) and position == "down": #both wrists above right shoulder
                    position="up"
                    bothArmsAbove = False
            ...
            text = f"Tennis Backhand Count: {count}"
